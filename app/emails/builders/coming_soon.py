...
from app.emails.images import fetch_and_attach_image, fetch_and_attach_small_thumbnail, truncate_text
...
def resolve_view(view):
    """The Coming Soon view requested on the builder item. '' (the default, and
    anything unrecognized) keeps each layout's own treatment."""
    view = (view or '').strip().lower()
    return view if view in VIEWS else ''

def _view_thumb(poster_url, msg_root, cid_name, base_url, view, hosted_images_enabled, hosted_base_url):
    if not poster_url:
        return None
    height = _AGENDA_THUMB_HEIGHT if view == 'agenda' else _CALENDAR_THUMB_HEIGHT
    return fetch_and_attach_small_thumbnail(poster_url, msg_root, cid_name, base_url, height=height,
...
def sonarr_events(episodes, msg_root, view, base_url="", cid_prefix="cs-tv", hosted_images_enabled=False, hosted_base_url="", with_posters=True, kind="", limit=0):
    ...
    for i, group in enumerate(sonarr_groups(episodes, kind, limit)):
        ...
        eps = group['episodes']
        first_ep = eps[0]
        day = _parse_release_date(group['air_date'])
        if day is None:
            continue
        title = series.get('title') or first_ep.get('title', 'Unknown')
        season = group['season']
        if len(eps) >= 2:
            subtitle = f"Season {season} ({len(eps)} episodes)" if season is not None else f"New episodes ({len(eps)})"
        else:
...
def render_view_html(events, theme_colors, view, title, container=None):
    """Calendar or agenda HTML for a set of events, or None when there is
    nothing datable to place. container wraps the table in section chrome;
    the default is the legacy card the poster grid uses."""
    if container is None:
        def container(inner):
            return section_container_html(theme_colors, title, inner)
    # Air dates and the *arr release dates are local-day values, so the
    # today/tomorrow labels and the today highlight key off the local date.
